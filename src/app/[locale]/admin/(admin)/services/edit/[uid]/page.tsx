...
import { fetchServicesByUid } from "@/db/queries/services";
import { updateServicesByUid } from "@/actions/servicesActions";
...
const SectionsEditPage: React.FC<SectionsEditPageProps> = ({ params }) => {
    ...
    const t = useTranslations("SERVER.MAILER");
    ...
    useEffect(() => {
        const loadData = async () => {
            const dataItems = await fetchServicesByUid(params.uid) || [];
            if (dataItems.length > 0) {
                const { languages, schema, initialValues } = await prepareFormDataFromDb(params.uid, servicesFields, dataItems);
    ...
    const onSubmit: SubmitHandler<FormInputType> = async (data) => {
        try {
            const result = await updateServicesByUid(params.uid, data);
