...
import { fetchPagesByUid } from "@/db/queries/pages";
import { updatePagesByUid } from "@/actions/pagesActions";
...
const SectionsEditPage: React.FC<SectionsEditPageProps> = ({ params }) => {
    ...
    useEffect(() => {
        const loadData = async () => {
            const dataItems = await fetchPagesByUid(params.uid) || [];
            if (dataItems.length > 0) {
                const { languages, schema, initialValues } = await prepareFormDataFromDb(params.uid, sectionsFields, dataItems);
    ...
    const onSubmit: SubmitHandler<FormInputType> = async (data) => {
        try {
            const result = await updatePagesByUid(params.uid, data);
    ...
    const breadcrumbs = [
        ...
        { title: "Sections", url: extractPathUpTo(pathname, "sections") },
        { title: `Edit section`, url: "" },
    ];

    return (
        <>
            <AdminBreadcrumbs items={breadcrumbs} />
            <div className={`flex-1 bg-white rounded-lg mx-3 mb-3   ${isLoading || noData  ? "flex justify-center items-center" : "items-start"} p-5 lg:p-8`}>
                {isLoading ? (
                    <Spinner />
                ) : noData ? (
