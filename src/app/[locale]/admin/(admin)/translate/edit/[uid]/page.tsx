...
import { fetchTranslatesByUid } from "@/db/queries/translate";
import { updateTranslateByUid } from "@/actions/translateActions";

interface TranslateEditPageProps {
    params: {
        uid: string;
    };
};

const TranslateEditPage: React.FC<TranslateEditPageProps> = ({ params }) => {
    ...
    useEffect(() => {
        const loadData = async () => {
            const dataItems = await fetchTranslatesByUid(params.uid) || [];
            if (dataItems.length > 0) {
                const { languages, schema, initialValues } = await prepareFormDataFromDb(params.uid, translateFields, dataItems);
    ...
    const onSubmit: SubmitHandler<FormInputType> = async (data) => {
        try {
            const result = await updateTranslateByUid(params.uid, data);
    ...
    const breadcrumbs = [
        ...
        { title: "Translate", url: extractPathUpTo(pathname, "translate") },
        { title: `Edit translate`, url: "" },
    ];
...
export default TranslateEditPage;
