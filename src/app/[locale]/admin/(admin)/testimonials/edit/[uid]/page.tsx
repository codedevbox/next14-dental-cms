...
import { fetchTestimonialsByUid } from "@/db/queries/testimonials";
import { updateTestimonialsByUid } from "@/actions/testimonialsActions";
...
const SectionsEditPage: React.FC<SectionsEditPageProps> = ({ params }) => {
    ...
    useEffect(() => {
        const loadData = async () => {
            const dataItems = await fetchTestimonialsByUid(params.uid) || [];
            if (dataItems.length > 0) {
                const { languages, schema, initialValues } = await prepareFormDataFromDb(params.uid, testimonialsFields, dataItems);
    ...
    const onSubmit: SubmitHandler<FormInputType> = async (data) => {
        try {
            const result = await updateTestimonialsByUid(params.uid, data);
    ...
    const breadcrumbs = [
        ...
        { title: "Testimonials", url: extractPathUpTo(pathname, "testimonials") },
        { title: `Edit testimonial`, url: "" },
    ];
