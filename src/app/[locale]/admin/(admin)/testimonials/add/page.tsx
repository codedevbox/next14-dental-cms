...
import { addTestimonials } from "@/actions/testimonialsActions";

const TestimonialsAddPage: React.FC = () => {

    const pathname = usePathname();
    const servicesPath = extractPathUpTo(pathname, "testimonials");
    ...
    useEffect(() => {
        const loadData = async () => {
            const {  languages, schema, initialValues } = await prepareFormData(testimonialsFields);
            setLanguages(languages || []);
            if (schema) {
                setFormSchema(schema);
            }
            await setLastSortOrder(initialValues, "testimonials");
    ...
    const onSubmit: SubmitHandler<FormInputType> = async (data) => {
        try {
            const result = await addTestimonials(data);
